                                         ...
                                         CodedBitstreamFragment *au)
{
    ...
    while (ff_cbs_sei_find_message(ctx->common.output, au,
        ...
        if (a53_side_data) {
            uint8_t *old_a53_side_data = NULL;
            size_t old_a53_side_data_size = 0;
            err = av_packet_get_side_data(pkt, AV_PKT_DATA_A53_CC, old_a53_side_data, old_a53_side_data_size);
            if (err) {
                av_log(bsf, AV_LOG_ERROR, "Failed to get A/53 "
                        "side data from packet.\n");
